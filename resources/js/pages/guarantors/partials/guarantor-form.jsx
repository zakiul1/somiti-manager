 ...
 return <form onSubmit={submit} className="space-y-6">
   <FormSection title={t('guarantors.basicInfo')} description={t('guarantors.requiredHint')}>
     ...
     <FormGrid>
       ...
       <div><AppLabel htmlFor="gender">{t('guarantors.gender')}</AppLabel><AppSelect id="gender" value={form.data.gender ?? ''} onChange={(e)=>form.setData('gender', e.target.value)}><option value="">--</option><option value="male">{t('guarantors.male')}</option><option value="female">{t('guarantors.female')}</option><option value="other">{t('guarantors.other')}</option></AppSelect><FormError>{form.errors.gender}</FormError></div>
   ...
   <FormSection title="Identity Media" description="Upload guarantor photo and NID front/back with preview.">
    <div className="grid gap-4 lg:grid-cols-3">
      <PreviewCard label="Photo" file={form.data.photo} currentUrl={!form.data.remove_photo ? guarantor?.photo_url : null} remove={form.data.remove_photo} setRemove={(v)=>form.setData('remove_photo',v)} setFile={(v)=>form.setData('photo',v)} accept="image/jpeg,image/png" />
      <PreviewCard label="NID Front" file={form.data.nid_front} currentUrl={!form.data.remove_nid_front ? guarantor?.nid_front_url : null} remove={form.data.remove_nid_front} setRemove={(v)=>form.setData('remove_nid_front',v)} setFile={(v)=>form.setData('nid_front',v)} accept="image/jpeg,image/png,application/pdf" />
      <PreviewCard label="NID Back" file={form.data.nid_back} currentUrl={!form.data.remove_nid_back ? guarantor?.nid_back_url : null} remove={form.data.remove_nid_back} setRemove={(v)=>form.setData('remove_nid_back',v)} setFile={(v)=>form.setData('nid_back',v)} accept="image/jpeg,image/png,application/pdf" />
    </div>
   </FormSection>
