...
export default function ResetPassword({ token, email }) {
    ...
    return (
        <GuestLayout>
            ...
            <div className="mb-6">
                <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100">{t('auth.resetTitle')}</h2>
            ...
            <form onSubmit={submit} className="space-y-5">
                ...
                <div>
                    ...
                    <TextInput type="password" id="password_confirmation" name="password_confirmation" value={data.password_confirmation} className="mt-1 block w-full" autoComplete="new-password" onChange={(e) => setData('password_confirmation', e.target.value)} />
                    <InputError message={errors.password_confirmation} className="mt-2" />
                </div>
                <PrimaryButton className="w-full justify-center" disabled={processing}>{t('auth.resetButton')}</PrimaryButton>
            </form>
        </GuestLayout>
    );
}
