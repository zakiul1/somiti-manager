        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0">
                    <p className="break-words text-sm font-semibold text-slate-900 dark:text-slate-100">{label}</p>
                    {helper ? <p className="mt-1 break-words text-xs text-slate-500 dark:text-slate-400">{helper}</p> : null}
                </div>
                {currentUrl ? (
                    <label className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                        <input type="checkbox" checked={remove} onChange={(e) => onToggleRemove(e.target.checked)} />
                        {t('common.removeCurrentFile')}
                    </label>
                ) : null}
            </div>
            <div className="mt-4 overflow-hidden rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">
                {previewUrl ? <img src={previewUrl} alt={label} className="h-52 w-full object-cover" /> : <div className="flex h-52 items-center justify-center bg-slate-50 text-sm text-slate-400 dark:bg-slate-950 dark:text-slate-500">{t('common.noImageSelected')}</div>}
            </div>
